/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.app.view;

import com.app.controller.StudentController;
import com.app.model.Student;
import javax.swing.JOptionPane;

/**
 *
 * @author Administrator
 */
public class UpdateStudentRecord extends javax.swing.JFrame {

    /**
     * Creates new form UpdateStudentRecord
     */
    StudentController sc = new StudentController();

    public UpdateStudentRecord() {
        initComponents();
        sc.populateStudentComboBox(updatecbb1);
        sc.populateAYTermComboBox(updatecbb2);
        sc.populateScholarshipComboBox(updatecbb3);
        sc.populateProgramComboBox(updatecbb4);
        
        updatecbb1.setSelectedIndex(-1);
        updatecbb2.setSelectedIndex(-1);
        updatecbb3.setSelectedIndex(-1);
        updatecbb4.setSelectedIndex(-1);
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        jPanel2 = new javax.swing.JPanel();
        update = new javax.swing.JButton();
        jLabel3 = new javax.swing.JLabel();
        updateNameField = new javax.swing.JTextField();
        jLabel4 = new javax.swing.JLabel();
        jLabel5 = new javax.swing.JLabel();
        jLabel6 = new javax.swing.JLabel();
        updatecbb4 = new javax.swing.JComboBox<>();
        updatecbb1 = new javax.swing.JComboBox<>();
        updatecbb3 = new javax.swing.JComboBox<>();
        updateAddressField = new javax.swing.JTextField();
        jLabel8 = new javax.swing.JLabel();
        jLabel9 = new javax.swing.JLabel();
        jLabel10 = new javax.swing.JLabel();
        jLabel11 = new javax.swing.JLabel();
        updateCampusField = new javax.swing.JTextField();
        updateContactField = new javax.swing.JTextField();
        updateYearField = new javax.swing.JTextField();
        updateSectionField = new javax.swing.JTextField();
        jLabel14 = new javax.swing.JLabel();
        jLabel15 = new javax.swing.JLabel();
        updatecbb2 = new javax.swing.JComboBox<>();
        jPanel6 = new javax.swing.JPanel();
        next = new javax.swing.JButton();
        jPanel3 = new javax.swing.JPanel();
        enrollmentCreate = new javax.swing.JButton();
        enrollmentcbb1 = new javax.swing.JComboBox<>();
        jLabel12 = new javax.swing.JLabel();
        jLabel13 = new javax.swing.JLabel();
        enrollmentcbb3 = new javax.swing.JComboBox<>();
        jPanel7 = new javax.swing.JPanel();
        jLabel2 = new javax.swing.JLabel();
        gobackupmenu = new javax.swing.JButton();
        jLabel1 = new javax.swing.JLabel();
        jLabel17 = new javax.swing.JLabel();
        jPanel1 = new javax.swing.JPanel();
        jLabel16 = new javax.swing.JLabel();

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);
        getContentPane().setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        jPanel2.setBackground(new java.awt.Color(255, 255, 255));
        jPanel2.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        update.setBackground(new java.awt.Color(204, 102, 0));
        update.setFont(new java.awt.Font("Verdana", 1, 36)); // NOI18N
        update.setForeground(new java.awt.Color(255, 255, 255));
        update.setText("UPDATE");
        update.setBorder(javax.swing.BorderFactory.createEmptyBorder(1, 1, 1, 1));
        update.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                updateActionPerformed(evt);
            }
        });
        jPanel2.add(update, new org.netbeans.lib.awtextra.AbsoluteConstraints(1030, 460, 220, 80));

        jLabel3.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel3.setText("ENTER STUDENT ID:");
        jPanel2.add(jLabel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 20, -1, -1));

        updateNameField.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        updateNameField.setToolTipText("");
        jPanel2.add(updateNameField, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 170, 440, 40));

        jLabel4.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel4.setText("UPDATED A.Y-TERM:");
        jPanel2.add(jLabel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 220, -1, -1));

        jLabel5.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel5.setText("UPDATED SCHOLARSHIP:");
        jPanel2.add(jLabel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 330, -1, -1));

        jLabel6.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel6.setText("UPDATED PROGRAM CODE:");
        jPanel2.add(jLabel6, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 430, -1, -1));

        updatecbb4.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        jPanel2.add(updatecbb4, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 480, 440, 40));

        updatecbb1.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        jPanel2.add(updatecbb1, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 70, 440, 40));

        updatecbb3.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        jPanel2.add(updatecbb3, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 380, 440, 40));

        updateAddressField.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        updateAddressField.setToolTipText("");
        jPanel2.add(updateAddressField, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 170, 720, 40));

        jLabel8.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel8.setText("UPDATED ADDRESS:");
        jPanel2.add(jLabel8, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 120, -1, -1));

        jLabel9.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel9.setText("UPDATED YEAR LEVEL:");
        jPanel2.add(jLabel9, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 220, -1, -1));

        jLabel10.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel10.setText("UPDATED CAMPUS:");
        jPanel2.add(jLabel10, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 430, -1, -1));

        jLabel11.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel11.setText("UPDATED SECTION:");
        jPanel2.add(jLabel11, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 330, -1, -1));

        updateCampusField.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        updateCampusField.setToolTipText("");
        jPanel2.add(updateCampusField, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 480, 390, 40));

        updateContactField.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        updateContactField.setToolTipText("");
        updateContactField.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                updateContactFieldActionPerformed(evt);
            }
        });
        jPanel2.add(updateContactField, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 70, 390, 40));

        updateYearField.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        updateYearField.setToolTipText("");
        jPanel2.add(updateYearField, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 270, 390, 40));

        updateSectionField.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        updateSectionField.setToolTipText("");
        jPanel2.add(updateSectionField, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 380, 390, 40));

        jLabel14.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel14.setText("UPDATED CONTACT NUMBER:");
        jPanel2.add(jLabel14, new org.netbeans.lib.awtextra.AbsoluteConstraints(530, 20, -1, -1));

        jLabel15.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel15.setText("UPDATED STUDENT NAME:");
        jPanel2.add(jLabel15, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 120, -1, -1));

        updatecbb2.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        jPanel2.add(updatecbb2, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 270, 440, 40));

        getContentPane().add(jPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 100, 1280, 570));

        jPanel6.setBackground(new java.awt.Color(112, 0, 0));
        jPanel6.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        next.setFont(new java.awt.Font("OldSansBlack", 0, 24)); // NOI18N
        next.setForeground(new java.awt.Color(255, 255, 255));
        next.setText("NEXT >");
        next.setBorder(null);
        next.setBorderPainted(false);
        next.setContentAreaFilled(false);
        next.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                nextActionPerformed(evt);
            }
        });
        jPanel6.add(next, new org.netbeans.lib.awtextra.AbsoluteConstraints(1140, 0, 160, 50));

        jPanel3.setBackground(new java.awt.Color(255, 255, 255));
        jPanel3.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        enrollmentCreate.setBackground(new java.awt.Color(51, 51, 51));
        enrollmentCreate.setFont(new java.awt.Font("Verdana", 1, 36)); // NOI18N
        enrollmentCreate.setForeground(new java.awt.Color(255, 255, 255));
        enrollmentCreate.setText("CREATE");
        enrollmentCreate.setBorder(javax.swing.BorderFactory.createEmptyBorder(1, 1, 1, 1));
        jPanel3.add(enrollmentCreate, new org.netbeans.lib.awtextra.AbsoluteConstraints(1030, 460, 220, 80));

        enrollmentcbb1.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        enrollmentcbb1.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Item 1", "Item 2", "Item 3", "Item 4" }));
        jPanel3.add(enrollmentcbb1, new org.netbeans.lib.awtextra.AbsoluteConstraints(460, 170, 390, 40));

        jLabel12.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel12.setText("STUDENT ID:");
        jPanel3.add(jLabel12, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 180, -1, -1));

        jLabel13.setFont(new java.awt.Font("Cambria", 1, 36)); // NOI18N
        jLabel13.setText("ENTER A.Y-TERM:");
        jPanel3.add(jLabel13, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 230, -1, -1));

        enrollmentcbb3.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        enrollmentcbb3.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Item 1", "Item 2", "Item 3", "Item 4" }));
        jPanel3.add(enrollmentcbb3, new org.netbeans.lib.awtextra.AbsoluteConstraints(460, 230, 390, 40));

        jPanel7.setLayout(new java.awt.BorderLayout());
        jPanel3.add(jPanel7, new org.netbeans.lib.awtextra.AbsoluteConstraints(470, 430, -1, -1));
        jPanel3.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(410, 610, 80, 200));

        jPanel6.add(jPanel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 100, 1280, 570));

        gobackupmenu.setFont(new java.awt.Font("OldSansBlack", 0, 24)); // NOI18N
        gobackupmenu.setForeground(new java.awt.Color(255, 255, 255));
        gobackupmenu.setText("Go back to Update Menu");
        gobackupmenu.setBorderPainted(false);
        gobackupmenu.setContentAreaFilled(false);
        gobackupmenu.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                gobackupmenuActionPerformed(evt);
            }
        });
        jPanel6.add(gobackupmenu, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 0, 300, 50));
        jPanel6.add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(100, 0, 30, 50));

        jLabel17.setIcon(new javax.swing.ImageIcon(getClass().getResource("/menu.png"))); // NOI18N
        jPanel6.add(jLabel17, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 0, 30, 50));

        getContentPane().add(jPanel6, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 670, 1290, 50));

        jPanel1.setBackground(new java.awt.Color(112, 0, 0));
        jPanel1.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        jLabel16.setBackground(new java.awt.Color(255, 255, 255));
        jLabel16.setFont(new java.awt.Font("SansSerif", 1, 48)); // NOI18N
        jLabel16.setForeground(new java.awt.Color(255, 255, 255));
        jLabel16.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        jLabel16.setText("Update Student Record");
        jPanel1.add(jLabel16, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 0, 1260, 100));

        getContentPane().add(jPanel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 1280, 100));

        setSize(new java.awt.Dimension(1296, 759));
        setLocationRelativeTo(null);
    }// </editor-fold>//GEN-END:initComponents

    private void updateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateActionPerformed
        UpdateStudentRecord USR = new UpdateStudentRecord();
        UpdateScheduleRecord USCR = new UpdateScheduleRecord();
        UpdateWindow updateW = new UpdateWindow();
        
        Student studentvar = new Student();
        try {
            int res = JOptionPane.showConfirmDialog(this, "Are you sure you want to update record for Student?", "Confirm", JOptionPane.YES_NO_OPTION);
            if (res == JOptionPane.YES_OPTION) {
                if (!updateNameField.getText().equals("")
                        || !updateContactField.getText().equals("")
                        || !updateAddressField.getText().equals("")
                        || !updateYearField.getText().equals("")
                        || !updateSectionField.getText().equals("")
                        || !updateCampusField.getText().equals("")){
                    
                    String selectedStudentStr = (String) updatecbb1.getSelectedItem();
                    int selectedStudent = Integer.parseInt(selectedStudentStr);
                    
                    String selectedAYTerm = (String) updatecbb2.getSelectedItem();
                    String selectedScholarship = (String) updatecbb3.getSelectedItem();
                    String selectedProgram = (String) updatecbb4.getSelectedItem();
                    
                    studentvar.setStudentID(selectedStudent);
                    studentvar.setStudentName(updateNameField.getText());
                    studentvar.setAYTerm(selectedAYTerm);
                    studentvar.setScholarship(selectedScholarship);
                    studentvar.setProgramCode(selectedProgram);
                    studentvar.setContactNumber(updateContactField.getText());
                    studentvar.setAddress(updateAddressField.getText());
                    studentvar.setYearLevel(updateYearField.getText());
                    studentvar.setSection(updateSectionField.getText());
                    studentvar.setCampus(updateCampusField.getText());

                    sc.updateStudent(studentvar);
                   
                    JOptionPane.showMessageDialog(null, "Record for Student successfully updated!");
                } else {
                    JOptionPane.showMessageDialog(null, "Field/s cannot be blank!", "Error", JOptionPane.ERROR_MESSAGE);
                }
                
                int result = JOptionPane.showConfirmDialog(this, "Do you want to update another record for Student?", "Confirmation", JOptionPane.YES_NO_OPTION);
                if (result == JOptionPane.YES_OPTION) {
                    dispose();
                    USR.show();
                } else if (result == JOptionPane.NO_OPTION) {
                    JOptionPane.showMessageDialog(null, "You will now return to the update menu");
                    updateW.show();
                }
            } else if (res == JOptionPane.NO_OPTION) {
                USR.show();
            }
        } catch (Exception e) {
        }

        dispose();
    }//GEN-LAST:event_updateActionPerformed

    private void updateContactFieldActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateContactFieldActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_updateContactFieldActionPerformed

    private void nextActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_nextActionPerformed
        UpdateScheduleRecord USCR = new UpdateScheduleRecord();
        USCR.show();
        dispose();
    }//GEN-LAST:event_nextActionPerformed

    private void gobackupmenuActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_gobackupmenuActionPerformed
        UpdateWindow updateW = new UpdateWindow();
        updateW.show(); //display frame called

        dispose();
    }//GEN-LAST:event_gobackupmenuActionPerformed

    /**
     * @param args the command line arguments
     */
    public static void main(String args[]) {
        /* Set the Nimbus look and feel */
        //<editor-fold defaultstate="collapsed" desc=" Look and feel setting code (optional) ">
        /* If Nimbus (introduced in Java SE 6) is not available, stay with the default look and feel.
         * For details see http://download.oracle.com/javase/tutorial/uiswing/lookandfeel/plaf.html 
         */
        try {
            for (javax.swing.UIManager.LookAndFeelInfo info : javax.swing.UIManager.getInstalledLookAndFeels()) {
                if ("Nimbus".equals(info.getName())) {
                    javax.swing.UIManager.setLookAndFeel(info.getClassName());
                    break;
                }
            }
        } catch (ClassNotFoundException ex) {
            java.util.logging.Logger.getLogger(UpdateStudentRecord.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (InstantiationException ex) {
            java.util.logging.Logger.getLogger(UpdateStudentRecord.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (IllegalAccessException ex) {
            java.util.logging.Logger.getLogger(UpdateStudentRecord.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (javax.swing.UnsupportedLookAndFeelException ex) {
            java.util.logging.Logger.getLogger(UpdateStudentRecord.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        //</editor-fold>
        //</editor-fold>

        /* Create and display the form */
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new UpdateStudentRecord().setVisible(true);
            }
        });
    }

    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton enrollmentCreate;
    private javax.swing.JComboBox<String> enrollmentcbb1;
    private javax.swing.JComboBox<String> enrollmentcbb3;
    private javax.swing.JButton gobackupmenu;
    private javax.swing.JLabel jLabel1;
    private javax.swing.JLabel jLabel10;
    private javax.swing.JLabel jLabel11;
    private javax.swing.JLabel jLabel12;
    private javax.swing.JLabel jLabel13;
    private javax.swing.JLabel jLabel14;
    private javax.swing.JLabel jLabel15;
    private javax.swing.JLabel jLabel16;
    private javax.swing.JLabel jLabel17;
    private javax.swing.JLabel jLabel2;
    private javax.swing.JLabel jLabel3;
    private javax.swing.JLabel jLabel4;
    private javax.swing.JLabel jLabel5;
    private javax.swing.JLabel jLabel6;
    private javax.swing.JLabel jLabel8;
    private javax.swing.JLabel jLabel9;
    private javax.swing.JPanel jPanel1;
    private javax.swing.JPanel jPanel2;
    private javax.swing.JPanel jPanel3;
    private javax.swing.JPanel jPanel6;
    private javax.swing.JPanel jPanel7;
    private javax.swing.JButton next;
    private javax.swing.JButton update;
    private javax.swing.JTextField updateAddressField;
    private javax.swing.JTextField updateCampusField;
    private javax.swing.JTextField updateContactField;
    private javax.swing.JTextField updateNameField;
    private javax.swing.JTextField updateSectionField;
    private javax.swing.JTextField updateYearField;
    private javax.swing.JComboBox<String> updatecbb1;
    private javax.swing.JComboBox<String> updatecbb2;
    private javax.swing.JComboBox<String> updatecbb3;
    private javax.swing.JComboBox<String> updatecbb4;
    // End of variables declaration//GEN-END:variables
}
